...
// LOGIN UND REGISTRIERUNG (Explizit an window binden)
...
window.startAsGuest = function () {
    ...
    // 1. Zurücksetzen auf die Standard-Konfiguration (Deep Copy!)
    subjectsConfig = JSON.parse(JSON.stringify(defaultSubjectsConfig));

    // 2. Daten leeren
    savedData = {};

    // 3. UI-Update aufrufen
    updateAppUI(false, true);
...
// REINE CLOUD-SPEICHERUNG
async function saveToCloud() {
    try {
        const { data: { session } } = await supabaseClient.auth.getSession();
        if (session && session.user) {
            // Wir suchen jetzt OHNE .single(), um Fehler bei Leerstand zu vermeiden
            ...
            // Prüfe, ob das Array existiert und nicht leer ist
...
// Wrapper-Funktionen ohne LocalStorage
function saveData() {
    berechneGesamtSchnitt();
    if (isGuestMode) {
        console.log("Gast-Modus aktiv: Speichern übersprungen.");
        return; // Einfach abbrechen, damit nichts in die Cloud geht
    }
    saveToCloud();
}
...
function berechneGesamtSchnitt() {
    ...
    document.getElementById("zeugnisdurchschnittkomma").innerText = kommaformat(kommaSchnittVal);
    document.getElementById("zeugnisdurchschnittecht").innerText = kommaformat(echtSchnittVal);
}
...
document.addEventListener('DOMContentLoaded', async () => {
    const { data: { session } } = await supabaseClient.auth.getSession();
    if (session) {
        // Der Listener wird beim Laden automatisch feuern, aber wir helfen kurz nach
        isGuestMode = false;
    } else {
        updateAppUI(false, false);
    }
...
window.renderModalSubjectList = function () {
    ...
    tempConfig.forEach((sub, idx) => {
        ...
        row.className = "edit-subject-row"; // Nutzt jetzt das Grid aus dem CSS
        row.innerHTML = `
            <div>
